...
class Server
{
    ...
    // check if is window OS
    public function iswin() : bool
    {
        // @var bool $isWindow
        $isWindow = false;

        if (strtolower(PHP_SHLIB_SUFFIX) == 'dll') $isWindow = true;
    ...
    public static function out($text, $code = null)
    {
        ...
        if ($code === null) $code = $ass->ansii('reset');
     ...
     * @return void
     * 
     * Start a php server on a new port or existing port
     */
    public static function serve(array $arg)
    {
        $ass = self::getInstance();
        ...
        // has port??
        if (isset($arg[0])) if (is_numeric($arg[0])) $port = $arg[0];
        ...
        bgExec("php -S localhost:{$port} " . __DIR__ . '/server.php');
    ...
    /**
     * @method Server get_mime
     * @param string $filepath
     * @return string
     */
    public static function get_mime(string $filepath) : string
    {
        static $mimeTypes;

        // get types
        if (is_null($mimeTypes)) $mimeTypes = self::MimeTypes();
    ...
    /**
     * @method Server loadFilesFromRoot
     * @return mixed
     */
    public static function loadFilesFromRoot()
    {
        // check for file
        ...
        if (is_file($documentRoot . '/' . $cleanRequestUrl)) :
            ...
            if (file_exists($documentRoot . '/index.php')) include_once $documentRoot . '/index.php';
     ...
     * @return Server
     */
    private static function getInstance() : Server
    {
        static $instance;
...
if ($_SERVER['SCRIPT_FILENAME'] == basename(__FILE__)) :
    ...
    Server::serve(array_splice($_SERVER['argv'], 1));

else:

    // server started
    Server::loadFilesFromRoot();
